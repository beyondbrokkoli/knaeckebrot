...
return function(SlideCaches, ActiveSlide, EngineState, Box_X, Box_Y, Box_Z, Box_NX, Box_NY, Box_NZ, MainCamera, ScreenPtr, ZBuffer)
    return function(CANVAS_W, CANVAS_H, HALF_W, HALF_H, MasterAlpha)
        ...
        -- --- THE EPSILON SNAP (The clean fix) ---
        -- If the scale is within 0.5% of native size, lock it to exactly 1.0
        if abs(draw_scale - 1.0) < 0.005 then
            draw_scale = 1.0
            -- When locked to 1:1, we must also snap the center coordinates
            -- to pure integers to prevent sub-pixel grid misalignment.
            cx = floor(cx + 0.5)
            cy = floor(cy + 0.5)
        end
